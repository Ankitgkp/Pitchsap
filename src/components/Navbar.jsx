...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed left-1/2 -translate-x-1/2 z-50 rounded-2xl border border-white/10 backdrop-blur-xl flex items-center justify-between transition-all duration-300 ${scrolled ? "top-2 w-[90%] max-w-5xl px-4 py-2" : "top-4 w-[95%] max-w-6xl px-6 py-3"} bg-surface/80`}>
      ...
      <a href="#login" className="hidden md:inline-block px-5 py-2 rounded-full bg-primary hover:bg-primary-dark text-white text-sm font-medium transition-all shadow-[0_3px_0_var(--color-primary-dark)] active:shadow-[0_0px_0_var(--color-primary-dark)] active:translate-y-[3px]" style={{ fontFamily: "'Aeonik Pro', sans-serif" }}>
        Login
      </a>
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-2 rounded-2xl border border-white/10 bg-surface/95 backdrop-blur-xl p-6 flex flex-col gap-4 md:hidden">
          ...
          <a href="#login" className="px-5 py-2 rounded-full bg-primary text-white text-sm font-medium text-center transition-all shadow-[0_3px_0_var(--color-primary-dark)] active:shadow-[0_0px_0_var(--color-primary-dark)] active:translate-y-[3px]" style={{ fontFamily: "'Aeonik Pro', sans-serif" }}>
            Login
          </a>
        </div>
      )}
